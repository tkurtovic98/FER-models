...
PublicTestloader = torch.utils.data.DataLoader(
    PublicTestset, batch_size=opt.bs, shuffle=False, num_workers=1)
PrivateTestset = FER2013(split='PrivateTest', transform=transform_test)
PrivateTestloader = torch.utils.data.DataLoader(
    PrivateTestset, batch_size=opt.bs, shuffle=False, num_workers=1)


# Training


def train(epoch, state: TrainingState):
    print('\nEpoch: %d' % epoch)
    net.train()
    train_loss = 0
    correct = 0
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        train(epoch, state)
        PublicTest(epoch, state)
        PrivateTest(epoch, state)
        plot_progress(state)
